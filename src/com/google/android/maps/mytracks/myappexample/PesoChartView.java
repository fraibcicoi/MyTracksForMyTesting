 ...
 */
public class PesoChartView extends View {
  ...
  public static final int WEIGHT_SERIES = 0;
  public static final int GOAL_WEIGHT_SERIES = 1;
  ...
  private final PesoChartValueSeries[] series = new PesoChartValueSeries[2];
   ...
   */
  public PesoChartView(Context context) {
    super(context);

    series[WEIGHT_SERIES] = new PesoChartValueSeries(context,
        Integer.MIN_VALUE,
        Integer.MAX_VALUE,
    ...
    series[GOAL_WEIGHT_SERIES] = new PesoChartValueSeries(context,
        0,
        Integer.MAX_VALUE,
 ...
 if(index<2)
  ...
  public void addDataPoints(ArrayList<double[]> dataPoints) {
    synchronized (chartData) {
      ...
      for (int i = 0; i < dataPoints.size(); i++) {
        ...
        for (int j = 0; j < 2; j++) {
          Log.d("test2", "series.lenght è "+series.length);
  ...
  private void drawDataSeries(Canvas canvas) {
    ...
    for (PesoChartValueSeries chartValueSeries : series) {
 ...
 private void mydrawDataSeries(Canvas canvas) {
    Log.d("test2", "in teoria dovrei disegnare");
   
        series[0].drawPath(canvas);
        series[1].drawPath(canvas);
      //  series[2].drawPath(canvas);
        
    //    series[0].getInterval();
  ...
  private void drawSeriesTitles(Canvas canvas) {
    ...
    for (int i = 0; i < series.length; i++) {
     PesoChartValueSeries chartValueSeries = series[i];
  ...
  private void drawXAxisMarker(
   ...
   // Date date = new Date();
    marker=dateFormat.format(value+ ((TemporaryData)(getContext().getApplicationContext())).getxAxis());
    
    
    canvas.rotate(30, getX(value), topBorder + effectiveHeight + spacing + rect.height());
  ...
  private void drawYAxis(Canvas canvas) {
    ...
    for (int i = 0; i < series.length; i++) {
     ...
     PesoChartValueSeries chartValueSeries = series[index];
 ...
 //   for (int i = 0; i < series.length; i++) {
    //  int index = series.length - 1 - i;
     PesoChartValueSeries chartValueSeries = series[0];
  //    if (chartValueSeries.isEnabled() && chartValueSeries.hasData()) {
        markerXPosition -= drawYAxisMarkers(chartValueSeries, canvas, markerXPosition) + spacer;
        
   //     PesoChartValueSeries chartValueSeries1 = series[1];
     //   chartValueSeries1.);
 ...
 //   }
   ...
   */
  private float drawYAxisMarkers(PesoChartValueSeries chartValueSeries, Canvas canvas, int xPosition) {
  ...
  private float drawYAxisMarker(
     PesoChartValueSeries chartValueSeries, Canvas canvas, int xPosition, int yValue) {
  ...
  private void drawPaths() {
    ...
    for (int i = 0; i < chartData.size(); i++) {
      ...
      for (int j = 0; j < series.length; j++) {
       ...
       PesoChartValueSeries chartValueSeries = series[j];
...
*/
  ...
  private void drawPaths() {
    ...
    for (int i = 0; i < chartData.size(); i++) {
      double[] dataPoint = chartData.get(i);
      PesoChartValueSeries chartValueSeries0 = series[0];
       
      {
        double value = dataPoint[1];
        if (Double.isNaN(value)) {
          continue;
        }
        
        Path path = chartValueSeries0.getPath();
        int x = getX(dataPoint[0]);
        int y = getY(chartValueSeries0, value);
        if (!hasMoved[0]) {
          hasMoved[0] = true;
          path.moveTo(x, y);
        } else {
      ...
      {
        double value = dataPoint[2];
        if (Double.isNaN(value)) {
          continue;
        }
        
       PesoChartValueSeries chartValueSeries = series[1];
        Path path = chartValueSeries.getPath();
        int x = getX(dataPoint[0]);
        int y = getY(chartValueSeries0, value);
        if (!hasMoved[0]) {
  ...
  private void updateDimensions() {
   ...
   // }
    PesoChartValueSeries chartValueSeries0=series[0];
    chartValueSeries0.updateDimension();
    
    PesoChartValueSeries chartValueSeries1=series[1];
    chartValueSeries1.updateDimension();
   ...
   * @param value the value
   */
  private int getY(PesoChartValueSeries chartValueSeries, double value) {
